		...
		if (ListUtil.isEmpty(args) || args[0].matches("^(remov|delet)e$")) {
			...
			try {
				...
				if (maskData.getType().isBlock()) {
					...
					itemData.setItemMeta(itemMeta);
					player.setItemInHand(itemData);
					this.getLog().message(sender, "The block in your hand now has an {{0}} block mask!", maskData.getType(), (StringUtil.notEmpty(mask) ? StringUtil.format(" (Previously {{0}})", mask) : ""));
				} else
