...
public class Config extends net.netcoding.niftybukkit.yaml.Config {
	...
	public Config(JavaPlugin plugin) {
		super(plugin, "config");
		ItemDatabase database = NiftyBukkit.getItemDatabase();
		...
		this.blacklists.put("spawned", database.parse(blacklisted));
		this.blacklists.put("creative", database.parse(blacklisted));
		this.blacklists.put("placement", database.parse("7,8,9,10,11,51,52,119,120,137,138,259,373:[16388,16420,16452,16424,16426,16428,16456,16458,16460],381,383:[51,52,54-62,65,66],384,385,397:1,401,407,422"));
	}
	...
	public boolean isBlacklisted(Player player, ItemStack stack, String list) {
		...
		if (stack != null && stack.getType() != Material.AIR) {
			...
			if ("store".equals(list))
				...
				for (ItemData item : this.blacklists.get(list)) {
